...
| `list_topics` | List all indexed topics and categories with document counts. Returns `{ topics, timing_ms }`. Each topic retains `category`, `topic`, `file_count`, `last_updated`, and `titles`, and adds `documents: [{ title, path }]` where `title` may be `null`. Use `path` directly with `get_document` (recommended workflow: `list_topics()` → `get_document(path)`). | (none) |
| `get_document` | Get the full content and metadata of a document by its relative path. **Direct retrieval by path** from an in-memory document index (no search, embeddings, or vector lookup). | `path` (e.g. `"deep-dive/mcp/overview.md"`) |
...
- **MCP performance instrumentation**: Every MCP tool response includes a `timing_ms` object by default (stage-level breakdown for `search`, `get_document`, and `list_topics`). Disable with `[instrumentation].timing_enabled = false`. See [docs/perf-instrumentation.md](docs/perf-instrumentation.md).
- **Live-sync file watcher**: `kb-mcp serve` spawns a `notify`-based watcher by default (`[watch].enabled = true`, 500 ms debounce). Manual saves, `git pull`, and external scripts are re-indexed incrementally on the same Mutex-guarded resources used by MCP tools, so concurrent triggers are serialized. The in-memory document index is updated in parallel so `get_document(path)` stays a fast path lookup. Disable with `--no-watch` or `[watch].enabled = false`.
