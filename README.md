...
| `list_topics` | List all indexed topics and categories with document counts. Returns `{ topics, timing_ms }`. Each topic retains `category`, `topic`, `file_count`, `last_updated`, and `titles`, and adds `documents: [{ title, path }]`. Use `path` directly with `get_document` (recommended workflow: `list_topics()` → `get_document(path)`). | (none) |
