...
| `src/server.rs` | `rmcp::ServerHandler` impl. Dispatches six MCP tools. `search` routes to `db.search_hybrid` and wraps the result in a `SearchResponse` with `low_confidence` / `match_spans` / `filter_applied` (BREAKING in v0.3.0; see CHANGELOG). MCP tool JSON responses include optional `timing_ms` (see `src/timing.rs`, `docs/perf-instrumentation.md`). |
| `src/timing.rs` | Monotonic stage timers and `timing_ms` serialization helpers for MCP tool responses. Controlled by `[instrumentation].timing_enabled` in `kb-mcp.toml` (default: on). |
