...
| `list_topics` | index 済みの全トピック / カテゴリと文書数を列挙。`{ topics, timing_ms }` を返す。各トピックは従来の `category` / `topic` / `file_count` / `last_updated` / `titles` に加え、`documents: [{ title, path }]` を含む。`path` はそのまま `get_document` に渡せる（推奨フロー: `list_topics()` → `get_document(path)`） | (なし) |
